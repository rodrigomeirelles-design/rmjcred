...
const createDbMock = (): SqliteDatabase => {
  const mockStmt = {
    get: () => ({ count: 0, total: 0 }),
  ...
  const mockDb = {
    ...
    prepare: () => mockStmt,
    transaction: (fn: any) => {
      const transFunc = (...args: any[]) => {
        try {
          return fn(...args);
        } catch (e) {
          console.error("Erro na transação mockada:", e);
        }
      };
      transFunc.default = transFunc;
      transFunc.deferred = transFunc;
      transFunc.immediate = transFunc;
      transFunc.exclusive = transFunc;
      return transFunc;
    },
  };
...
try {
  ...
  safeAddColumn("empresas", "fonte", "TEXT", "'Empresa'");
  ...
  console.warn("SQLite não pôde ser inicializado, usando mock:", e);
  db = createDbMock();
}
